...
interface IEVault {
    // ERC4626-like interfaces as used by Euler V2 vaults
    ...
    function borrow(uint256 assets, address receiver) external returns (uint256 borrowed);

    // Repay underlying assets on behalf of a specific account (debt owner)
    function repay(uint256 assets, address onBehalfOf) external returns (uint256 repaid);
}
