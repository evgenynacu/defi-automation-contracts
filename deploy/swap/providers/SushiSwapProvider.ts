...
export class SushiSwapProvider implements ISwapProvider {
  ...
  public async getQuote(params: SwapParams): Promise<SwapResult> {
    ...
    const url = `https://api.sushi.com/swap/v5/${params.chainId}?tokenIn=${this.toSushi(params.fromToken)}&tokenOut=${this.toSushi(params.toToken)}&amount=${params.swapAmount.toString()}&to=${params.vault}&preferSushi=true&includeTransaction=true`
